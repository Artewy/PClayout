const timer = () => {
    ...
    const updateTimer = () => {
        const date = new Date();
        const dateDeadline = new Date(2026, 5, 12).getTime();
        const timeRemaining = (dateDeadline - date) / 1000;


        const days = Math.floor(Math.abs(timeRemaining) / 60 / 60 / 24);
        const hours = Math.floor((Math.abs(timeRemaining) / 60 / 60) % 24);
        const minutes = Math.floor((Math.abs(timeRemaining) / 60) % 60);
        const seconds = Math.floor(Math.abs(timeRemaining) % 60);
        ...
        const allTimerElements = [
            daysBlock, hoursBlock, minutesBlock, secondsBlock,
            daysBlock.nextElementSibling, hoursBlock.nextElementSibling,
            minutesBlock.nextElementSibling, secondsBlock.nextElementSibling
        ];

        allTimerElements.forEach(element => {
            if (element) {
                element.classList.toggle('timer__expired', isExpired);
            }
        });
